...
export default function AutopostSettings() {
	...
	useEffect(() => {
		Promise.all([
			...
			.then(([settingsRes, usersRes]) => {
				...
				const newValues = {
					getmini_api_key: data.has_api_key ? "••••••••••••" : "",
					pre_prompt: data.pre_prompt || "",
					model: data.model || "getmini-2.5-flash",
	...
	const onSubmit = async (data) => {
		...
		try {
			...
			const isNewKey = data.getmini_api_key &&
				data.getmini_api_key.trim() !== "" &&
				!data.getmini_api_key.startsWith("••");

			if (isNewKey) {
				payload.getmini_api_key = data.getmini_api_key.trim();
			}
		...
		<div className="space-y-6">
			<Card className="max-w-2xl">
				<CardHeader className="flex flex-row items-center justify-between space-y-0">
					...
					<Button type="submit" form="autopost-settings-form" disabled={loading}>
						{loading ? (
							<>
				...
				<CardContent>
					<Form {...form}>
						<form id="autopost-settings-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
							<FormField
								control={form.control}
								name="getmini_api_key"
								render={({ field }) => (
									<FormItem>
										<FormLabel>{"API Key de Gemini"}</FormLabel>
										<FormControl>
											<Input
												type="password"
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>{"Autor por Defecto"}</FormLabel>
										<FormControl>
											<select
								...
								render={({ field }) => (
									<FormItem>
										...
										<p className="text-sm text-muted-foreground mt-1">
											{"Este texto se combinará con el tema para generar el artículo."}
										</p>
									</FormItem>
								)}
							/>
